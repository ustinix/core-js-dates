...
function formatDate(date) {
  ...
  const minutes = String(d.getUTCMinutes());
  const seconds = String(d.getUTCSeconds());
...
function getWeekNumberByDate(date) {
  ...
  const daysFromStart = (date - StartOfTheYear) / oneDay;
  const startDayWeek =
...
function getNextFridayThe13th(date) {
  ...
  for (let month = 0; month < 12; month += 1) {
    const day13 = new Date(currentYear, month, 13);
    nextFridayThe13th = day13.getDay() === 5 ? day13 : nextFridayThe13th;
    if (nextFridayThe13th) break;
  }
